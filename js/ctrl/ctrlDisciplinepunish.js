...
angular.module('myApp.ctrl')
    .controller('disciplinepunishCtrl', ['$state', '$scope', '$rootScope', 'disciplinepunishService', '$filter', 'Upload', '$timeout', function ($state, $scope, $rootScope, disciplinepunishService, $filter, Upload, $timeout) {
        ...
        $scope.getPublicNotificationList = function (x) {
            disciplinepunishService.getPublicNotificationList(x).then(function (res) {
        ...
        $scope.getOrgListInfo = function () {
            disciplinepunishService.getOrgList(loginfo.token).then(function (res) {
                if (res.data.error && res.data.error.code == 4999) {
                    swal({
                            title: "登录授权已过期,请点击确认重新登录",
                            type: "warning",
        ...
        $scope.addPubInfo = function () {
            disciplinepunishService.addInfo($scope.addInfo).then(function (res) {
                if (res.data.error && res.data.error.code == 4999) {
                    swal({
                            title: "登录授权已过期,请点击确认重新登录",
                            type: "warning",
        ...
        $scope.getOneInfo = function (id) {
            disciplinepunishService.getOnePublicNotificationInfo(loginfo.token, id).then(function (res) {
                if (res.data.error && res.data.error.code == 4999) {
                    swal({
                            title: "登录授权已过期,请点击确认重新登录",
                            type: "warning",
        ...
        $scope.updateUserInfo = function () {
            ...
            disciplinepunishService.updateInfo(loginfo.token, $scope.publicNotificationDetails).then(function (res) {
                if (res.data.error && res.data.error.code == 4999) {
                    swal({
                            title: "登录授权已过期,请点击确认重新登录",
                            type: "warning",
        ...
        $scope.startDelete = function (id) {
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        disciplinepunishService.deleteInfo(loginfo.token, id).then(function (res) {
                            swal({
                                title: "数据已删除!",
                                timer: 2000,
                                showConfirmButton: true
